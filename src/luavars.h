//-----------------------------------------------------------------------bl-
//--------------------------------------------------------------------------
// 
// QEPPS: Quadratic eigenvalue problem parameter sweeper
//
// Copyright (C) 2014 Lab for Active Nano Devices, UT ECE 
// Developed by Ian Williamson 
// Supervised by Dr. Zheng Wang 
//
//-----------------------------------------------------------------------el-
// 
// Defs for the variable and table keys in the LUA state
// 
//--------------------------------------------------------------------------
//--------------------------------------------------------------------------

#ifndef QEPPS_LUAVARS
#define QEPPS_LUAVARS

#define LUA_array_parameters "parameters"
#define LUA_array_options "options"

#define LUA_table_matricies "matricies"
#define LUA_key_matrix_E "E"
#define LUA_key_matrix_D "D"
#define LUA_key_matrix_K "K"

#define LUA_subkey_data "data"
#define LUA_subkey_func "func"

#endif
